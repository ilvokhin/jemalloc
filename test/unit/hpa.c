...
TEST_BEGIN(test_demand_purge_slack) {
	...
	hpa_shard_opts_t opts = test_hpa_shard_opts_default;
	opts.deferral_allowed = true;
	/* Demand sliding window interval is 10 seconds. */
	opts.demand_interval_ms = 10 * 1000;
	 ...
	 * keep 11 * 0.1 (hpa_demand_slack_mult) = 1.1 dirty hugepages, but we
...
TEST_BEGIN(test_demand_purge_strict) {
	test_skip_if(!hpa_supported());
	...
	/* Demand sliding window interval is 10 seconds. */
	opts.demand_interval_ms = 10 * 1000;
	/* Demand slack multiplier is zero. */
	opts.demand_slack_mult = FXP_INIT_PERCENT(0);
	...
	expect_zu_eq(0, ndefer_dehugify_calls, "Dehugified too early");
	/*
	 * Peak demand within sliding window is 16 hugepages, to purge anything just
	 * yet.
	 */
	expect_zu_eq(0, ndefer_purge_calls, "Purged too early");
	 ...
	 * hpa_demand_slack_mult = 0, so we allowed to keep 11 * 0 = 0 dirty
